...
/**
 * A class which contains some common graph algorithms for any graph that
...
public class GraphAlgorithms {
    ...
    public static <E> List<Vertex<E>> breadthFirstSearch(GraphADT<E> graph, Vertex<E> startVertex) {
    ...
    // at the given vertex
    public static <E> List<Vertex<E>> depthFirstSearch(GraphADT<E> graph, Vertex<E> startVertex) {
        if (!graph.containsVertex(startVertex)) {
            throw new IllegalArgumentException();
        }
        // create list to hold vertices as they are encountered
    ...
    public static <E> Set<Vertex<E>> depthFirstSetSearch(GraphADT<E> graph, Vertex<E> startVertex) {
        ...
        while (!processingStack.isEmpty()) {
            Vertex<E> firstVertex = processingStack.peek();
            
            Iterator<Vertex<E>> iterator = firstVertex.adjacentVertices().iterator();
            boolean found = false;
            while (!found && iterator.hasNext()) {
                Vertex<E> adjacentVertex = iterator.next();
    ...
    public static <E> Set<Vertex<E>> recursiveDepthFirstSearch(GraphADT<E> graph, Vertex<E> startVertex) {
        Set<Vertex<E>> vertices = new HashSet<>();
        if (!graph.containsVertex(startVertex)) {
            throw new IllegalArgumentException();
        }
        
        recursionDFS(startVertex, vertices);
        return vertices;
    }
    
    public static <E> void recursionDFS(Vertex<E> startVertex, Set<Vertex<E>> vertices) {
        Iterator<Vertex<E>> iterator = startVertex.adjacentVertices().iterator();
        vertices.add(startVertex);
    ...
    private static <E> void extendPath(Map<Vertex<E>, Set<Path<E>>> paths, Path<E> path) {
...
//        System.out.println("Performing Depth-first search from Mel:");
//        List<Vertex<String>> list = depthFirstSearch(graph, melbourne);
...
//        System.out.println("there are " + paths.get(samoa).size() + " such paths");

        System.out.println("recursive DFS");
        Set<Vertex<String>> set = recursiveDepthFirstSearch(graph, melbourne);
        for (Vertex<String> s : set)
        {
