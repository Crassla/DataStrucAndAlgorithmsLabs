...
public class BinarySearchTree<E> extends AbstractSet<E>
    ...
    private boolean leftRootRotaion()
    {
        if (rootNode.rightChild != null)
        {
            ...
            {
                BinaryTreeNode rightLeftChild = rootNode.leftChild;
                root.rightChild = rightLeftChild;
            }
            else
            {
    ...
    private boolean rightRootRotaion()
    {
        if (rootNode.leftChild != null)
        {
            BinaryTreeNode root = rootNode;
            ...
            if (rootNode.rightChild != null)
            {
                BinaryTreeNode rightRightChild = rootNode.rightChild;
                root.leftChild = rightRightChild;
            }
            else
            {
                root.leftChild = null;
    ...
    public static void main(String[] args) {  // create the binary search tree
        ...
        tree.leftRootRotaion();
        System.out.println("Left Rotation: " + tree);
        tree.leftRootRotaion();
        System.out.println("Left Rotation: " + tree);
        tree.rightRootRotaion();
...
//        System.out.println("last element in subtree: " + subtree.last());
    ...
    public List<E> doTraversal(int order) {

        List<E> elements = new ArrayList<>();

        switch (order) {
            case 0:
                inOrder(elements, rootNode);
                break;
            case 1:
                preOrder(elements, rootNode);
                break;
            case 2:
                postOrder(elements, rootNode);
                break;
            case 3:
                levelOrder(elements, rootNode);
                break;
        }
    ...
    public void inOrder(List<E> element, BinaryTreeNode node) {
        recInOrder(element, node);
        return;
    }

    private void recInOrder(List<E> element, BinaryTreeNode current) {
        if (current == null)
            return;
        recInOrder(element, current.leftChild);
        element.add(current.element);
        recInOrder(element, current.rightChild);
    }

    public void preOrder(List<E> element, BinaryTreeNode node) {
        recPreOrder(element, node);
        return;
    }
    
    private void recPreOrder(List<E> element, BinaryTreeNode current) {
        if (current == null)
            return;
        element.add(current.element);
        recPreOrder(element, current.leftChild);
        recPreOrder(element, current.rightChild);
    }

    public void postOrder(List<E> element, BinaryTreeNode node) {
        recPostOrder(element, node);
        return;
    }
    
    private void recPostOrder(List<E> element, BinaryTreeNode current) {
        if (current == null)
            return;
        recPostOrder(element, current.leftChild);
        recPostOrder(element, current.rightChild);
        element.add(current.element);
    }

    public void levelOrder(List<E> element, BinaryTreeNode node) {
